...
// 简短人设，放在消息开头
const GROUP_PREFIX = '[群聊模式-保持简洁自然，不要自称AI]';
...
function recordGroupMessage(groupId, userId, message, nickname) {
  ...
  const filtered = history.filter(h => h.time > Date.now() - SESSION_EXPIRE_MS);
...
function callHermes(message, sessionKey, sessions, isGroup, groupId, nickname) {
  try {
    ...
    // 清理回复中可能残留的人设前缀
    reply = reply.replace(/\[群聊模式[^\]]*\]/g, '').trim();
...
console.log('');
console.log('========================================');
console.log('   Hermes Agent Worker v9');
console.log('========================================');
console.log('');
console.log('配置:');
console.log('  私聊: mimo-v2.5-pro');
console.log('  群聊: mimo-v2.5（按群ID，附带记录）');
console.log('  超时: 3天');
console.log('');
